      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="rounded-xl border bg-card shadow-sm">
          ...
          <ul className="divide-y">
            {alertas.map((a) => (
              <li
                key={a.id}
                className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 p-4 hover:bg-muted/30"
              >
                <div
                  className={`grid h-10 w-10 shrink-0 place-items-center rounded-lg ${a.severidad === "alta" ? "bg-destructive/10 text-destructive" : a.severidad === "media" ? "bg-warning/20 text-warning-foreground" : "bg-info/10 text-info"}`}
                >
        ...
        <aside className="rounded-xl border bg-card p-5 shadow-sm">
          ...
          <div className="relative h-80 overflow-hidden rounded-lg border bg-gradient-to-br from-primary/5 via-muted to-info/5">
            ...
            <svg viewBox="0 0 200 200" className="absolute inset-0 h-full w-full opacity-40">
              ...
              <rect
                ...
                stroke="oklch(0.5 0.16 255)"
                strokeWidth="1.5"
                fill="oklch(0.9 0.012 250)"
              />
              <text x="100" y="100" textAnchor="middle" fontSize="6" fill="oklch(0.32 0.12 255)">
                EDIFICIO CONTROL
              </text>
            </svg>
            {[
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Visualización geográfica de incidentes activos en el complejo fronterizo.
          </p>
        </aside>
      </div>
    </AdminLayout>
