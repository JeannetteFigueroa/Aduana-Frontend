...
function Dashboard() {
  return (
    <AdminLayout
      ...
      subtitle="Resumen operativo del paso fronterizo · 11 jun 2026"
    >
        ...
        <div className="rounded-xl border bg-card p-5 shadow-sm lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex gap-3 text-xs">
              ...
              <span className="flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-full bg-success" /> Salidas
              </span>
            </div>
          </div>
          <div className="h-64">
        ...
        <div className="rounded-xl border bg-card p-5 shadow-sm">
          ...
          <div className="h-56">
            <ResponsiveContainer>
              <PieChart>
                <Pie
                  data={distribucionNacionalidad}
                  dataKey="value"
                  innerRadius={45}
                  outerRadius={75}
                  paddingAngle={2}
                >
                  {distribucionNacionalidad.map((_, i) => (
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        <div className="rounded-xl border bg-card shadow-sm lg:col-span-2">
          <div className="flex items-center justify-between border-b p-5">
            ...
            <a href="/admin/viajero" className="text-xs font-medium text-primary hover:underline">
              Ver todos →
            </a>
          </div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            <div className="mt-3 space-y-2.5">
              {["Cabina 1", "Cabina 2", "Cabina 3", "Cabina 4", "Cabina 5", "Cabina 6"].map(
                (c, i) => (
                  <div key={c} className="flex items-center justify-between text-sm">
                    <span className="flex items-center gap-2">
                      <span
                        className={`h-2 w-2 rounded-full ${i === 4 ? "bg-destructive" : i === 2 ? "bg-warning" : "bg-success"}`}
                      />
                      {c}
                    </span>
                    <span className="text-xs text-muted-foreground">
                      {i === 4 ? "Sin señal" : `${Math.floor(Math.random() * 12) + 2} en cola`}
                    </span>
                  </div>
                ),
              )}
      ...
      <div className="mt-4 rounded-xl border bg-card p-5 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          ...
          <a href="/admin/alertas" className="text-xs font-medium text-primary hover:underline">
            Centro de alertas →
          </a>
        </div>
        <ul className="divide-y">
          {alertas.slice(0, 3).map((a) => (
            <li key={a.id} className="flex items-center gap-3 py-3">
              <AlertTriangle
                className={`h-4 w-4 shrink-0 ${a.severidad === "alta" ? "text-destructive" : a.severidad === "media" ? "text-warning-foreground" : "text-info"}`}
              />
