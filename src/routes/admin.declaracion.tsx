...
function Declaracion() {
  ...
  return (
    <AdminLayout title="Declaración SAG digital" subtitle="Productos de origen vegetal, animal o derivados">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-4">
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            <h3 className="font-semibold">1. ¿Transporta alguno de estos productos?</h3>
            <p className="text-sm text-muted-foreground">Seleccione todas las categorías que correspondan.</p>
            ...
            <div className="space-y-2">
              {items.map(it => (
                <div key={it.id} className="grid grid-cols-1 gap-2 rounded-md border bg-background p-3 sm:grid-cols-[2fr_1fr_1fr_auto]">
                  <input defaultValue={it.nombre} placeholder="Nombre del producto" className="h-9 rounded-md border bg-card px-3 text-sm outline-none focus:ring-2 focus:ring-ring/50" />
                  <input defaultValue={it.cantidad} placeholder="Cantidad" className="h-9 rounded-md border bg-card px-3 text-sm outline-none focus:ring-2 focus:ring-ring/50" />
                  <input defaultValue={it.origen} placeholder="País de origen" className="h-9 rounded-md border bg-card px-3 text-sm outline-none focus:ring-2 focus:ring-ring/50" />
                  <button onClick={() => setItems(items.filter(x => x.id !== it.id))} className="grid h-9 w-9 place-items-center rounded-md border hover:bg-destructive/10 hover:text-destructive">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              ))}
          ...
          <div className="flex justify-end gap-2">
            <button className="rounded-md border bg-card px-5 py-2.5 text-sm font-medium hover:bg-muted">Guardar borrador</button>
            <button className="rounded-md bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground hover:bg-primary/90">Enviar declaración</button>
          </div>
        </div>
        ...
        <aside className="space-y-4">
          ...
          <div className="rounded-xl border bg-card p-5 shadow-sm">
            ...
            <dl className="mt-3 space-y-2 text-sm">
              <div className="flex justify-between"><dt className="text-muted-foreground">Categorías</dt><dd className="font-medium">{selected.length}</dd></div>
              <div className="flex justify-between"><dt className="text-muted-foreground">Productos</dt><dd className="font-medium">{items.length}</dd></div>
              <div className="flex justify-between"><dt className="text-muted-foreground">Estado</dt><dd className="font-medium text-warning-foreground">Borrador</dd></div>
            </dl>
          </div>
        </aside>
      </div>
